...
import org.groovymud.object.behaviours.*;
...
if(args == null || args.size() == 0){
	...
	if(args[0] == ("me")){
		...
		if(target == null){
			target = ((Container)source).getItems(item);		
		}
		...
		if(target == null || target.size() == 0){
			target = container.getItems(item);
		}
		if(target == null){
...
if(target != null){
	...
	def item = args != null ? args[0] : null
    	...
    	if(target.size() > 0){
			...
			{ 
    		    obj -> 
    		    LookBehaviour look = obj.getLookBehaviour();				
    		    look.doLook(source, obj); 
			}
    	}else{
    	    LookBehaviour look = target.getLookBehaviour()
			
	    	look.doLook(source, target);
	    }
